...
export async function createCommentFlow(input: {
  ...
  const reviewRequired = Boolean(input.forcePending || postContext.settings.requireCommentReview)
  const reviewNote = input.forcePending
    ? (input.reviewNote || "AI 审核建议进入人工复核")
